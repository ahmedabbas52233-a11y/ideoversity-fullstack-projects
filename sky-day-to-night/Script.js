...
class ElysiumAudio {
    ...
    playBird() {
        ...
        [0, 0.1, 0.18].forEach((offset, i) => {
            ...
            osc.frequency.exponentialRampToValueAtTime(base * (1.2 + i * 0.1), now + offset + 0.05);
    ...
    setIntensity(nightness) {
        ...
        this.masterGain.gain.setTargetAtTime(0.2 + nightness * 0.2, this.ctx.currentTime, 0.5);
    }
}
...
function createDeer() {
    ...
    const brown = new THREE.MeshStandardMaterial({ color: 0x8B5A2B, flatShading: true, roughness: 0.9 });
    const dark = new THREE.MeshStandardMaterial({ color: 0x5D3A1A, flatShading: true });
    ...
    // Ears
    [[-0.08, 0.08], [0.08, -0.08]].forEach(([ox, oz]) => {
    ...
    // Antlers
    const antlerMat = new THREE.MeshStandardMaterial({ color: 0x4a3728, flatShading: true });
    [[-0.12, 0.1], [0.12, -0.1]].forEach(([ox, oz]) => {
        const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.035, 0.5, 4), antlerMat);
        stem.position.set(0.85 + ox, 2.95, oz);
        stem.rotation.z = ox > 0 ? -0.25 : 0.25;
        group.add(stem);
        
        // Branch
        const branch = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.02, 0.2, 4), antlerMat);
    ...
    [[0.88, 0.06], [0.88, -0.06]].forEach(([x, z]) => {
    ...
    [[-0.4, -0.2], [0.4, -0.2], [-0.4, 0.2], [0.4, 0.2]].forEach(([x, z], i) => {
...
function createRabbit() {
    const group = new THREE.Group();
    const white = new THREE.MeshStandardMaterial({ color: 0xe8e0d8, flatShading: true, roughness: 0.9 });
    const pink = new THREE.MeshStandardMaterial({ color: 0xf0a0a0, flatShading: true });
    ...
    // Ears
    [[0.22, 0.08], [0.22, -0.08]].forEach(([x, z]) => {
        const ear = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.06, 0.4, 6), white);
        ear.position.set(0.35, 0.85, z);
        ear.rotation.x = z > 0 ? 0.15 : -0.15;
        ear.rotation.z = -0.1;
        group.add(ear);
        
        const inner = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.03, 0.25, 6), pink);
...
function createOwl() {
    const group = new THREE.Group();
    const feather = new THREE.MeshStandardMaterial({ color: 0x6b4423, flatShading: true, roughness: 0.95 });
    const belly = new THREE.MeshStandardMaterial({ color: 0xc4a574, flatShading: true });
    const dark = new THREE.MeshStandardMaterial({ color: 0x2a1f15, flatShading: true });
    ...
    [[-0.12, 0.12], [-0.12, -0.12]].forEach(([x, z]) => {
        const white = new THREE.Mesh(new THREE.SphereGeometry(0.14, 8, 8), eyeWhite);
        white.position.set(x, 2.9, z);
        group.add(white);
        
        const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.07, 8, 8), eyePupil);
        pupil.position.set(x - 0.03, 2.9, z);
        group.add(pupil);
    });
    
    // Beak
    const beak = new THREE.Mesh(new THREE.ConeGeometry(0.04, 0.1, 4), new THREE.MeshStandardMaterial({ color: 0xe67e22 }));
    beak.position.set(0.32, 2.85, 0);
    beak.rotation.z = -Math.PI / 2;
    group.add(beak);
    
    // Wings
    [[0, 0.35], [0, -0.35]].forEach(([y, z]) => {
...
function createTree(x, z, scale) {
    const group = new THREE.Group();
    const trunkMat = new THREE.MeshStandardMaterial({ color: 0x5d4037, flatShading: true, roughness: 0.95 });
    const leafMat = new THREE.MeshStandardMaterial({ color: 0x2d7a3d, flatShading: true, roughness: 0.9 });
    
    const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.15 * scale, 0.22 * scale, 2.5 * scale, 6), trunkMat);
    trunk.position.y = 1.25 * scale;
    trunk.castShadow = true;
    group.add(trunk);
    
    const foliage1 = new THREE.Mesh(new THREE.ConeGeometry(1.2 * scale, 2 * scale, 6), leafMat);
    foliage1.position.y = 2.8 * scale;
    foliage1.castShadow = true;
    group.add(foliage1);
    
    const foliage2 = new THREE.Mesh(new THREE.ConeGeometry(0.9 * scale, 1.5 * scale, 6), leafMat);
...
class Elysium {
    ...
    initScene() {
        ...
        // Trees
        const treePositions = [
            [-15, 8, 1.2], [18, 12, 0.9], [-22, -5, 1.5], [25, -8, 1.1],
            [-8, 20, 0.8], [12, 22, 1.3], [-30, 5, 1.0], [30, 15, 0.85],
            [5, -20, 1.4], [-12, -18, 1.0], [20, -15, 1.2], [-25, -12, 0.9]
        ];
    ...
    updateTimeOfDay() {
        ...
        // Sky uniforms
        const sunDir = new THREE.Vector3(sunX, Math.max(0.05, sunY), -0.5).normalize();
        ...
        else if (t < 7) fogColor = dayColor.clone().lerp(duskColor, 1 - (t - 5) / 2);
        ...
        this.ambientLight.color.copy(ambientDay.clone().lerp(ambientNight, Math.min(1, nightness)));
    ...
    updateUI() {
        ...
        const dh = h % 12 || 12;
        document.getElementById('clock').textContent = `${dh}:${m.toString().padStart(2,'0')} ${ampm}`;
    ...
    animateAnimals(elapsed) {
        ...
        // Deer tail flick
        this.deer.children[this.deer.children.length - 1].rotation.z = 1.2 + Math.sin(elapsed * 3) * 0.1;
        
        // Rabbit hop (occasional)
        const hopPhase = (elapsed % 4) / 4;
        if (hopPhase < 0.15) {
            this.rabbit.position.y = -1.2 + Math.sin(hopPhase / 0.15 * Math.PI) * 0.4;
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof THREE === 'undefined') {
        document.querySelector('.loader p').textContent = 'Failed to load 3D engine. Please check your connection.';
