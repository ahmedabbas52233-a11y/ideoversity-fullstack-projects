...
function sciFunc(fn) {
  ...
  switch (fn) {
    ...
    case 'asin':
      result =
        angleMode === 'DEG' ? (Math.asin(val) * 180) / Math.PI : Math.asin(val);
      break;
    case 'acos':
      result =
        angleMode === 'DEG' ? (Math.acos(val) * 180) / Math.PI : Math.acos(val);
      break;
    case 'atan':
      result =
        angleMode === 'DEG' ? (Math.atan(val) * 180) / Math.PI : Math.atan(val);
      break;
    case 'log':
...
function progFunc(fn) {
  ...
  switch (fn) {
    case 'bin':
      current = val.toString(2);
      break;
    case 'oct':
      current = val.toString(8);
      break;
    case 'hex':
      current = val.toString(16).toUpperCase();
      break;
    case 'dec':
      current = val.toString(10);
      break;
    case 'and':
      chooseOperation('and');
      return;
